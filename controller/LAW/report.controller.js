...
const { caseCodeFinder } = require("../../utils/LAW/abuseDBRelator");
...
const lawReportPoster = async (req, res) => {
    ...
    try {
        ...
        if( !name || !age || !gender || !abuse || !evidence || !actionTaken || !actionWant || !email || !phone || !reportMeans || !who ) return res.status(402).json({error: 'All questions must be answered', success: false})
        ...
        res.status(201).json({success: true, message: 'Your Report has been received and will be looked into, kindly check your email for your case tracking code', caseCode: caseCode })
